import type {
  ...
  IExpiringPoliciesResponse,
  IRenewalPreviewResponse,
  IRenewalPolicyResponse,
  IRiskRuleListResponse,
  IRiskRuleResponse,
...
// V3 — Renewal API Functions

export async function getExpiringPolicies(): Promise<IExpiringPoliciesResponse> {
  return authFetch<IExpiringPoliciesResponse>('/api/v1/policies/expiring');
}

export async function getRenewalPreview(id: number): Promise<IRenewalPreviewResponse> {
  return authFetch<IRenewalPreviewResponse>(`/api/v1/policies/${id}/renewal-preview`);
}

export async function renewMyPolicy(id: number): Promise<IRenewalPolicyResponse> {
  return authFetch<IRenewalPolicyResponse>(`/api/v1/policies/${id}/renew`, {
    method: 'POST',
  });
}

// V4 — Risk Rules API Functions

export async function getRiskRules(): Promise<IRiskRuleListResponse> {
  return authFetch<IRiskRuleListResponse>('/api/v1/admin/risk-rules');
}

export async function createRiskRule(data: IRiskRuleCreateRequest): Promise<IRiskRuleResponse> {
  return authFetch<IRiskRuleResponse>('/api/v1/admin/risk-rules', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}

export async function updateRiskRule(id: number, data: IRiskRuleUpdateRequest): Promise<IRiskRuleResponse> {
  return authFetch<IRiskRuleResponse>(`/api/v1/admin/risk-rules/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  });
}
...
export async function deleteRiskRule(id: number): Promise<void> {
  return authFetch<void>(`/api/v1/admin/risk-rules/${id}`, { method: 'DELETE' });
}

export async function toggleRiskRule(id: number): Promise<IRiskRuleResponse> {
